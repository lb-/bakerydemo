...
- On every admin page, the Stimulus code is loaded with the default controllers added and an event `wagtail:stimulus-init` (bubbles:true) would be fired that other code could hook into. See https://github.com/lb-/bakerydemo/blob/ui-experiments/bakerydemo/ui/wagtail_hooks.py
  - This looks like a really good candidate, it does not pollute the global and also provides a way for us to avoid adding all controllers in the core JS if we want (we could always do a hybrid approach also). If we dispatch the event on `document.body` (not document) and the core Wagtail events use the `document.addEventListener('init', myRegisterFn, {capture:true})` - these event listeners will by default be run first, then the documentation can recommend adding the listener to `document` without capture. This also means there can be an ordered set of event listener gaps. If someone wants to run something BEFORE the wagtail listeners they can add event listener to window with capture or directly to document.body. or as a last resort you can run fully last by adding the event listener to the `window`. So the order will be; -1 `window/capture` (register something before Wagtail's are registered), 1 `document/capture` (core Wagtail ones registered), 2.`document` (custom overrides documented approach), 3. `window` (fallback available for additional layers on top).
